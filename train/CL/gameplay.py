...
def gameplay_fwpass(q_model, o_model, inputs, exp_config, word2i, train= True,
    ...
    # print(history.size())
    # print(inputs['history'].size())
    # print(inputs['history']); exit()
    history[:,0] = inputs['history'].squeeze()
    ...
    for q_idx in range(exp_config['max_no_qs']):
        ...
        if use_dataparallel and use_cuda:
            qgen_out = q_model.module.qgen.sampling(src_q=inputs['src_q'][mask_ind], encoder_hidden=encoder_hidden[_enc_mask],     
                visual_features=avg_img_features[mask_ind], greedy=True, beam_size=1)
        else:
            # print(mask_ind); print(_enc_mask); print(mask_ind.dim())
            qgen_out = q_model.qgen.sampling(src_q=inputs['src_q'][mask_ind], encoder_hidden=encoder_hidden[_enc_mask], 
                visual_features=avg_img_features[mask_ind], greedy=True, beam_size=1)
