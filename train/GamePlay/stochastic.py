...
# failed exp, not used in this work
...
def make_stochastic(module, rho_prior, dist='gaussian', 
    device='cuda'):
    ...
    for child in immediate_frozen_children:
        make_stochastic(child, rho_prior, dist=dist, device=device)
...
def isprob(module):
    check = False
    for layer_type in CONVERSION_MAP.values():
