...
class QGenSeq2Seq(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        **kwargs : dict
        ...
        word_logits = self.to_logits(rnn_hiddens.reshape(-1, self.qgen_args['hidden_dim'])).view(
            batch_size,
            self.qgen_args['max_tgt_length'],
            self.qgen_args['vocab_size']
        )
    ...
    def basicforward(self, embedding, rnn_state):
        ...
        rnn_hiddens.contiguous()
        logits = self.to_logits(rnn_hiddens.reshape(-1, self.qgen_args['hidden_dim'])).view(embedding.size(0), 1,
                                                                                         self.qgen_args['vocab_size'])
