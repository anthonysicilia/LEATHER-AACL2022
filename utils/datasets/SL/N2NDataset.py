...
from utils.datasets.SL.fairutils import process_annotations, downsample

from utils.create_subset import create_subset

class N2NDataset(Dataset):

    def __init__(self, split='train', fair=False, **kwargs):
        ...
        if fair:
            print('Old', len(self.n2n_data))
            mimgs, fimgs = process_annotations()
            mimgs, fimgs = downsample(mimgs, fimgs)
            mimgs = set(mimgs)
            fimgs = set(fimgs)
            new_data = []
            for idx, game in self.n2n_data.items():
                # print(game)
                imid = game['image_file'].split('_')[-1].split('.')[0].lstrip('0')
                if int(imid) in fimgs or int(imid) in mimgs:
                    new_data.append(game)
            self.n2n_data = {str(i) : game for i, game in enumerate(new_data)}
            print('New', len(self.n2n_data))
