...
class RndObjSampDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 0 in _data['objects']:
            ...
            wrong_obj = None
            while wrong_obj is None or wrong_obj == _data['target_obj']:
                wrong_obj = random.choice(range(len(_data['objects'][:list(_data['objects']).index(0)])))
            _data['wrong_obj'] = wrong_obj
        else:
            _data['target_obj'] = random.choice(range(len(_data['objects'][:])))
            wrong_obj = None
            while wrong_obj is None or wrong_obj == _data['target_obj']:
                wrong_obj = random.choice(range(len(_data['objects'][:])))
            _data['wrong_obj'] = wrong_obj
